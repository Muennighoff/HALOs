#!/bin/bash
# example usage:
# bash scripts/debug_qwen_8gpu_grpo_async_filtering_bs256.sh 0.05 1e-6 0.2
...
export CACHE_DIR=/data/niklas/HALOs/models/qwen15-online-grpo-filter-bs256-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler
...
for r in $(seq $NUM_ROUNDS -1 1); do
    LAST_EXP_NAME=qwen15-online-grpo-filter-bs256-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler-R${r}
...
while [ $ROUND -le ${NUM_ROUNDS} ]; do
    ...
    if [[ $NUM_SAMPLES -gt 0 ]]; then
        EXP_NAME=qwen15-online-grpo-filter-bs256-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler-binary-R${ROUND}
        ...
        accelerate launch --config_file accelerate_config/fsdp_8gpu_offload.yaml --num_processes $NUM_GPUS launch.py loss=grpo model=qwen train_datasets=[$SAMPLES_FILE] test_datasets=[$SAMPLES_FILE] exp_name=${EXP_NAME} \
            ...
            ++eval_every=999999 ++do_first_eval=false \
