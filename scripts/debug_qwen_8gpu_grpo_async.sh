...
while [ $ROUND -le ${NUM_ROUNDS} ]; do
    ...
    if [[ $NUM_SAMPLES -gt 0 ]]; then
        ...
        accelerate launch --config_file accelerate_config/fsdp_8gpu_offload.yaml --num_processes $NUM_GPUS launch.py loss=grpo model=qwen train_datasets=[$DATA_FILE] test_datasets=[$DATA_FILE] exp_name=${EXP_NAME} \
            ++cache_dir=${CACHE_DIR} \
