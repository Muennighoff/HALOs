#!/bin/bash
# example usage:
# bash scripts/debug_qwen_4gpu_grpo_async.sh 0.05 1e-6 0.2
...
export CACHE_DIR=/data/niklas/HALOs/models/qwen-online-grpo-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler
...
for r in $(seq $NUM_ROUNDS -1 1); do
    LAST_EXP_NAME=qwen-online-grpo-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler-R${r}
...
while [ $ROUND -le ${NUM_ROUNDS} ]; do
    ...
    CUDA_VISIBLE_DEVICES=2,3,4,5 python -m train.sample $CURRENT_CKPT \
    ...
    if [[ $NUM_SAMPLES -gt 0 ]]; then
        EXP_NAME=qwen-online-grpo-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler-binary-R${ROUND}
        ...
        accelerate launch --config_file accelerate_config/fsdp_4gpu.yaml --num_processes $NUM_GPUS launch.py loss=grpo model=qwen train_datasets=[$DATA_FILE] test_datasets=[$DATA_FILE] exp_name=${EXP_NAME} \
            ++cache_dir=${CACHE_DIR} \
