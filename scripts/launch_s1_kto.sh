...
#MODEL_PATH=simplescaling/s1.1-32B
MODEL_PATH=qfq/s1.1-32B-bf16
CKPT=/data/niklas/models/s1.1-32b-kto-v2
...
accelerate launch \
    ...
    --max_restarts 0 \
    launch.py loss=kto model=qwen train_datasets=[s1k_11] test_datasets=[s1k_11] exp_name=s1.1-32b-kto-v2 do_first_eval=false \
    ++cache_dir=/data/niklas/models \
    ++model.name_or_path=$MODEL_PATH ++model.tokenizer_name_or_path=Qwen/Qwen2.5-32B-Instruct \
...
python -m train.sample $CKPT --gpu_count 2 --output_file outputs/s11-32b-kto.json
"