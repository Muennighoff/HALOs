...
# example usage:
# sbatch scripts/qwen_4node_grpo_async.sh 0.05 1e-6 0.2
...
while [ $ROUND -le ${NUM_ROUNDS} ]; do
    ...
    if [[ $NUM_SAMPLES -gt 0 ]]; then
        ...
        accelerate launch --config_file accelerate_config/fsdp_2x8gpu.yaml launch.py loss=grpo model=qwen train_datasets=[$DATA_FILE] test_datasets=[$DATA_FILE] exp_name=${EXP_NAME} \
            ++cache_dir=${CACHE_DIR} \
