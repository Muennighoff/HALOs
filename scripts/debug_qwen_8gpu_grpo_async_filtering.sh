#!/bin/bash
# example usage:
# bash scripts/debug_qwen_8gpu_grpo_async_filtering.sh 0.05 1e-6 0.2
...
# TOTAL_PROMPTS=10240
# PROMPTS_PER_ROUND=1024  # Train on fewer examples before sampling ; # steps will be (1024 / (bs/group_size))
...
export CACHE_DIR=/data/niklas/HALOs/models/qwen15-online-grpo-filter-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler
...
for r in $(seq $NUM_ROUNDS -1 1); do
    LAST_EXP_NAME=qwen15-online-grpo-filter-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler-R${r}
...
while [ $ROUND -le ${NUM_ROUNDS} ]; do
    ...
    if [[ $NUM_SAMPLES -gt 0 ]]; then
        EXP_NAME=qwen15-online-grpo-filter-kl-${KL_COEF}-lr-${LR}-clip-${CLIP_RANGE}-deepscaler-binary-R${ROUND}
        ...
        # accelerate launch --config_file accelerate_config/fsdp_8gpu_offload.yaml --num_processes 8 launch.py loss=grpo model=qwen 'train_datasets=[/data/niklas/HALOs/models/qwen15-online-grpo-kl-0.05-lr-1e-6-clip-0.2-deepscaler/R1_data.json]' 'test_datasets=[/data/niklas/HALOs/models/qwen15-online-grpo-kl-0.05-lr-1e-6-clip-0.2-deepscaler/R1_data.json]' exp_name=qwen15-online-grpo-kl--lr--clip--deepscaler-binary-R1 ++cache_dir=/data/niklas/HALOs/models/qwen15-online-grpo-kl--lr--clip--deepscaler ++model.name_or_path=Qwen/Qwen2.5-1.5B-Instruct ++lr=1e-6 ++intermediate_checkpoints=true ++online=true ++weight_decay=0.0 ++beta1=0.9 ++beta2=0.95 ++eps=1e-5 ++warmup=0.10 ++loss.ppo_epochs=1 ++loss.cliprange=0.2 ++loss.lam=0.95 ++loss.gamma=1.0 ++loss.critic_coef=0.1 ++loss.KL_coef=0.05 ++n_epochs=1 ++model.batch_size=256 ++model.max_grad_norm=1 ++model.max_length=4096 ++model.max_prompt_length=1024 ++model.gradient_accumulation_steps=4 ++model.eval_batch_size=256 ++eval_every=999999 ++do_first_eval=false ++frac_unique_undesirable=0.0 ++model.load_from=Qwen/Qwen2.5-1.5B-Instruct
        accelerate launch --config_file accelerate_config/fsdp_8gpu_offload.yaml --num_processes $NUM_GPUS launch.py loss=grpo model=qwen train_datasets=[$SAMPLES_FILE] test_datasets=[$SAMPLES_FILE] exp_name=${EXP_NAME} \
            ++cache_dir=${CACHE_DIR} \
...
# accelerate launch --config_file accelerate_config/fsdp_8gpu_offload.yaml --num_processes 1 launch.py loss=grpo model=qwen 'train_datasets=[/data/niklas/HALOs/models/qwen15-online-grpo-filter-bs256-kl-0.05-lr-1e-6-clip-0.2-deepscaler/R1_samples.json]' 'test_datasets=[/data/niklas/HALOs/models/qwen15-online-grpo-filter-bs256-kl-0.05-lr-1e-6-clip-0.2-deepscaler/R1_samples.json]' exp_name=qwen15-online-grpo-filter-bs256-kl-0.05-lr-1e-6-clip-0.2-deepscaler-binary-R1 ++cache_dir=/data/niklas/HALOs/models/qwen15-online-grpo-filter-bs256-kl-0.05-lr-1e-6-clip-0.2-deepscaler ++model.name_or_path=Qwen/Qwen2.5-1.5B-Instruct ++lr=1e-6 ++intermediate_checkpoints=true ++online=true ++weight_decay=0.0 ++beta1=0.9 ++beta2=0.95 ++eps=1e-5 ++warmup=0.10 ++loss.ppo_epochs=1 ++loss.cliprange=0.2 ++loss.lam=0.95 ++loss.gamma=1.0 ++loss.critic_coef=0.1 ++loss.KL_coef=0.05 ++n_epochs=1 ++model.batch_size=256 ++model.max_grad_norm=1 ++model.max_length=4096 ++model.max_prompt_length=1024 ++model.gradient_accumulation_steps=4 ++model.eval_batch_size=256 ++eval_every=999999 ++do_first_eval=false ++frac_unique_undesirable=0.0 ++model.load_from=Qwen/Qwen2.5-1.5B-Instruct