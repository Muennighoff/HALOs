#!/bin/bash
# example usage:
# bash scripts/debug_qwen_1gpu_grpo_async.sh 0.05 1e-6 0.2
...
while [ $ROUND -le ${NUM_ROUNDS} ]; do
    ...
    CUDA_VISIBLE_DEVICES=6 python -m train.sample $CURRENT_CKPT \
    ...
    if [[ $NUM_SAMPLES -gt 0 ]]; then
        ...
        accelerate launch --num_processes 1 launch.py loss=grpo model=qwen train_datasets=[$DATA_FILE] test_datasets=[$DATA_FILE] exp_name=${EXP_NAME} \
            ++cache_dir=${CACHE_DIR} \
