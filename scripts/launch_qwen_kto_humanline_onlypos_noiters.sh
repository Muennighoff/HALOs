...
CKPT=/data/niklas/models/qwen-32b-kto-humanline-pos-noiters

# Run the training script using srun
srun --jobid=$SLURM_JOB_ID --nodes=$SLURM_JOB_NUM_NODES --ntasks-per-node=1 bash -c "
accelerate launch \
    --config_file accelerate_config/fsdp_2x8gpu_offload.yaml \
    ...
    --max_restarts 0 \
    launch.py loss=kto model=qwen train_datasets=[s1k_11] test_datasets=[s1k_11] exp_name=qwen-32b-kto-humanline-pos-noiters do_first_eval=false \
    ...
    ++model.attn_implementation=flash_attention_2 ++eval_every=999999 ++n_eval_examples=0 ++humanline=true ++frac_unique_undesirable=0.0 ++humanline_iters=1
