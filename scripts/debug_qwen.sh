...
--max_restarts 0 \
launch.py loss=sft model=qwen train_datasets=[s1k_11] test_datasets=[s1k_11] exp_name=qwen-05B-sft n_epochs=1 do_first_eval=false \
...
++model.batch_size=8 ++model.gradient_accumulation_steps=1 ++model.microbatch_size=1 ++model.eval_batch_size=8 \
++model.max_length=5000 ++model.max_prompt_length=5000 \
++model.attn_implementation=flash_attention_2 ++eval_every=999999 ++model.policy_dtype=float32
